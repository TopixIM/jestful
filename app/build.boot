...
(task-options!
  pom {:project     'topixim/jestful
       :version     +version+
       :description "tiye"
       :url         "https://github.com/topixim/jestful"
       :scm         {:url "https://github.com/topixim/jestful"}
...
(deftask dev! []
  ...
  (comp
    ...
    (reload :on-jsload 'jestful.core/on-jsload
            :cljs-asset-path ".")
    (cljs)
    (target)))
...
(deftask rsync []
  (with-pre-wrap fileset
    (sh "rsync" "-r" "target/" "tiye.me:repo/TopixIM/jestful" "--exclude" "main.out" "--delete")
    fileset))
